        ...
        $.ajax(api_issue_url, {
            ...
            success: function (issues, textStatus, jqXHR) {
                ...
                $.each(issues, function (i, issue) {
                    if (i == 0) {
                        $("#add").append("<div class='product-rating'><a href='" + issue.html_url + "#issue-" + issue.id + "' target='blank' class='tooltip--triangle' data-tooltip='It works'><div class='reaction' name='input[content]' type='submit' value='Like' count='" + issue.reactions["+1"] + "' >" + issue.reactions["+1"] + "</div></a><a href='" + issue.html_url + "' target='blank' class='tooltip--triangle' data-tooltip='It does not work'><div class='reaction' name='input[content]' type='submit' value='Dislike' count='" + issue.reactions["-1"] + "' >" + issue.reactions["-1"] + "</div></a></div>");
                        console.log(i); 
                    }}
                );
